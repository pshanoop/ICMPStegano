 ...
 * 
 * Compiler Option : Please compile this code with -pthread option.
 * 
...
#include <iostream>
#include <fstream> // file creation and read
#include <cmath>
#include <cstdio>
#include "msgqueue.h"
...
#define STEGANO_INIT 2 //TODO 
#define STEGANO_SEND 3
#define STEGANO_ECHO 4 //TODO
...
#define STEGANO_FMAX  4294967295 // Maximum file size that can be send thorugh (4GB) it is the size of
                                 // unsigned char
...
}cpacket;  // custom ICMP packet for steganography


//size of file for send or receive
typedef union {
    unsigned int bytes;        
//    struct b16{
...
public:    
    ...
    void display(void *buf,int bytes); // used only for testing
    void display(int test); // used only for testing
    

    void SetMode(const unsigned short mode); // TO set mode of steganography
    void SetFname(const char fname[]); // file for send/ receive
    void SetQid(long _in,long _out); // sets message queue id(s) for IPC
...
private:   
  ...
  void Icmp_Receive(void); // receives a ICMP request packet
